...
export const InstitutionDashboard: React.FC<InstitutionDashboardProps> = ({
          ...
          (newExpense: Expense) =>
            !prevExpenses.some(
              (existingExpense: Expense) =>
                existingExpense.expenseID === newExpense.expenseID
            )
        );
  ...
  const handleSaveState = async (projectID: number) => {
    ...
    try {
      ...
      console.log(
        `State for project ${projectID} successfully saved as ${state}`
      );
    } catch (error) {
  ...
  const generatePDF = () => {
    ...
    const projectExpenses = expenses.filter(
      (expense) => expense.projectID === selectedProject
    );
    ...
    // Set document title
    const title = project
      ? `Expenses for ${project.projectName}`
      : "Project Expenses";
    doc.text(title, 20, 20);

    // Calculate total cost
    const totalCost = projectExpenses.reduce(
        ...
        <Table className="min-w-full bg-muted/10 rounded-lg overflow-hidden">
          <TableHeader className="bg-muted/20">
            <TableRow>
              ...
              <TableHead className="p-2 text-left hidden sm:table-cell">
                Start Date
              </TableHead>
            ...
            {getCurrentPageProjects().map((project) => (
              ...
              >
                ...
                <TableCell className="p-2">
                  ${calculateTotalExpenses(project.projectID).toFixed(2)}
                </TableCell>
                <TableCell className="p-1">
                  <select
                    ...
                    onChange={(e) =>
                      handleStateChange(
                        project.projectID,
                        e.target.value as Project["state"]
                      )
                    }
