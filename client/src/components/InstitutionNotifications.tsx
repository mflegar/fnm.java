...
export function Notifications({ institution }: NotificationsProps) {
  ...
  const fetchNotifications = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/notification/institution/${institution.institutionID}`, {
        headers: {
  ...
  const handleAction = async (action: 'accept' | 'reject', notificationID: number, actorID: number) => {
    console.log(`${action} clicked for notification ${notificationID} and actor ${actorID}`);
    ...
    try {
      ...
      if (action === 'accept') {
        const postResponse = await fetch(`/api/institution/joininstitution/${actorID}`, {
          method: 'POST',
          headers: {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-2xl font-bold">Notifications to join Institution {institution.institutionName}</CardTitle>
              ...
              <TableBody>
                {notifications.map((notification) => (
                  <TableRow key={notification.notificationID}>
                    ...
                    <TableCell className="hidden lg:table-cell">{new Date(notification.createdAt).toLocaleString()}</TableCell>
                    <TableCell>{actors[notification.actorID] || "Loading..."}</TableCell>
                    <TableCell>{notification.institutionID}</TableCell>
                    <TableCell className="text-right">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleAction('accept', notification.notificationID, notification.actorID)}
                        className="mr-2"
                      >
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleAction('reject', notification.notificationID, notification.actorID)}
                      >
