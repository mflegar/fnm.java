...
export function AddExpense() {
  ...
  const fetchProjectID = async (projectName: string) => {
    ...
    try {
      ...
      showNotification('error', 'An error occurred while fetching the project data')
    }
  }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!description.trim() || !expense.trim() || isNaN(Number(expense)) || Number(expense) <= 0 || !projectID) {
      showNotification('error', 'Please provide a valid description, positive integer for the expense, and ensure project data is available')
    ...
    try {
      ...
      showNotification('error', 'An error occurred while submitting the expense')
    } finally {
      setIsSubmitting(false)
    }
  }
